...
# Bash function to convert one MARC-XML file to a Bibframe2 RDF file.
# Depends on the environment variables defined in ld4p_configure.sh
# Depends on installation of the loc marc2bibframe2 project.
# Usage:  loc_marc2bibframe2 {MRC_XML}
#
...
export LOC_M2B_PATH="${LD4P_LIB}/loc_marc2bibframe2"
export LOC_M2B_XSL="${LOC_M2B_PATH}/xsl/marc2bibframe2.xsl"
...
loc_marc2bibframe2 () {
    ...
    if [ ! -s "${MRC_RDF}" -o "${MRC_XML}" -nt "${MRC_RDF}" -o ${LD4P_MARCRDF_REPLACE} == true ]; then
        ...
        xsltproc --stringparam baseuri ${LD4P_BASEURI} ${LOC_M2B_XSL} ${MRC_XML} \
                1> ${MRC_RDF} \
                2>> ${LD4P_MARCRDF_LOG}
